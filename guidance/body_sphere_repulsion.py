...
def collision_response(
    ...
    if sphere_mesh_checked[batch_index, point_index, mesh_index] == 0:
        ...
        if got_dist:
            ...
            # Points AWAY from obstacle (Repulsive direction)
            # Handle Inside/Outside cases to ensure 'n' always points Outwards
            if inside < 0.0:
                # Inside: Center is deep inside, Closest is on surface.
                # We want to push towards surface (Outwards).
                # Vector: Closest - Center
                n = wp.normalize(closest_point - sphere_center_point)
            else:
                # Outside: Center is outside, Closest is on surface.
                # We want to push away from surface (Outwards).
                # Vector: Center - Closest
